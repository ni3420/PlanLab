...
export default function CreateDirectModel({ onCancel, isOpen, onOpenChange }: CreateTaskFormProps) {
  ...
  const isLoadingDependencies = loadingProjects || loadingMembers;

  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-xl p-0 overflow-hidden border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-xl sm:rounded-xl max-h-[90vh] overflow-y-auto no-scrollbar">
        <DialogHeader className="space-y-1.5 p-6 border-b border-neutral-100 dark:border-neutral-800/60 text-left">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value} disabled={isPending || isLoadingDependencies}>
                        <FormControl>
                          <SelectTrigger className="h-10 border-neutral-200 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-950/50 rounded-lg text-left text-sm w-full flex items-center justify-between px-3">
                            <SelectValue placeholder={isLoadingDependencies ? "Syncing layout..." : "Select project track"} />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800">
