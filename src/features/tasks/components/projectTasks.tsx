...
import { Kanban, Table2, Plus, ListTodo, Loader2 } from "lucide-react";
...
export default function ProjectTasksPage() {
  ...
  // Fetch the current project context parameters to show the project name dynamically
    ...
    return (
      <div className="flex h-[60vh] w-full items-center justify-center">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-xs text-neutral-400 font-medium">Synchronizing track metadata...</p>
        </div>
      </div>
    );
  }
    ...
    <>
      <div className="h-full flex flex-col space-y-6 p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-neutral-100 dark:border-neutral-800/60 pb-5">
          <div className="space-y-1 min-w-0">
            <div className="flex items-center gap-2">
              <div className="h-8 w-8 rounded-lg bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center text-neutral-900 dark:text-neutral-100 shrink-0">
                <ListTodo className="h-4.5 w-4.5 stroke-[2]" />
              </div>
              <h1 className="text-2xl font-bold tracking-tight text-neutral-900 dark:text-neutral-100 truncate">
                {project?.name || "Project Track"} Tasks
              </h1>
            </div>
            <p className="text-xs text-neutral-500 dark:text-neutral-400 truncate">
              Managing workflow execution vectors isolated within project track: <span className="font-mono bg-neutral-100 dark:bg-neutral-800/60 px-1 py-0.5 rounded text-[11px]">{projectId}</span>
            </p>
          </div>
        ...
        <div className="flex-1 w-full overflow-hidden">
          {viewMode === "kanban" ? (
            /* The KanbanBoard and TaskTable internally use useGetTasks which parses the current workspaceId and projectId automatically */
      ...
      {/* Task Creation Dialog Frame Context */}
